...
class PoseResNet(nn.Module):
    ...
    def __init__(self, block, layers, heads, head_conv, **kwargs):
        ...
        self.atten = ChannelAttention(256)
    ...
    def _make_deconv_layer(self, num_layers, num_filters, num_kernels, stride):
...
class ChannelAttention(nn.Module):
    def __init__(self, C):
        ...
        self.weight = torch.nn.Parameter(torch.FloatTensor(C), requires_grad=True)
    ...
    def forward(self, x):
        ...
        atten = self.sigmoid(self.weight)
        output = atten * x
        return output
