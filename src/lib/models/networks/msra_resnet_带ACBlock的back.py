...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        # Bottleneck的第一个conv和第三个conv是1*1的，不需要转成acblock。只有3*3的转
        # self.acblock1 = ACBlock(inplanes, planes, stride=self.stride, padding=1, kernel_size=1, deploy=False)
        self.acblock2 = ACBlock(planes, planes, stride=self.stride, padding=1, kernel_size=3, deploy=False)
        # self.acblock3 = ACBlock(planes, planes * self.expansion, stride=self.stride, padding=1, kernel_size=1, deploy=False)
